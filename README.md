...
- `run_carriage_bridge.sh` — скрипт для автосборки образа/воркспейса и запуска контейнера.
- `Dockerfile.carriage_bridge` — Docker-образ с предустановленными runtime-зависимостями (`xacro`, `pymodbus`, `pyserial`).
...
По умолчанию скрипт автоматически выбирает UART-порт в порядке:
`/dev/serial0` -> `/dev/ttyAMA0` -> `/dev/ttyS0`.
При необходимости можно указать порт вручную:

```bash
BRIDGE_PORT=/dev/serial0 ./run_carriage_bridge.sh
```

### 4. Что происходит при первом и повторном запуске

- **Первый запуск**:
  - автоматически собирается локальный Docker-образ `carriage-bridge:humble-rpi` (если его еще нет);
  - автоматически выполняется `colcon build` внутри контейнера (если отсутствует `ros2_ws/install/setup.bash`);
  - после этого запускается `ros2 launch carriage_bridge carriage_launch.py`.
- **Повторные запуски**:
  - образ и собранный workspace переиспользуются;
  - ручные `apt/pip` установки не требуются.
